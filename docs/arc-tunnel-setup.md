# Arc Tunnel 浏览器自动化（可选 / 实验性）

> ⚠️ **实验性功能**：Arc Tunnel 为可选外围增强模块，默认关闭。该功能需要安装 Chrome/Edge 扩展并授予较高权限，建议仅在隔离环境/测试浏览器中使用，不要在处理敏感数据（银行、邮箱、内部系统等）的浏览器中启用。
...
- `debugger` — 可通过 Chrome DevTools Protocol 控制所有标签页
- `cookies` — 读取所有网站的 Cookie
- `scripting` — 在所有页面注入脚本
- `<all_urls>` — 访问所有 URL
- `tabs` — 访问所有标签页信息

**风险说明**：上述权限组合意味着扩展可以读取所有网站的敏感信息、截取屏幕、记录用户输入。在企业环境中，IT 部门通常不允许自行安装未上架扩展，开发者模式加载也是临时性的（浏览器重启后需重新加载）。

**建议**：
- 仅在明确需要浏览器自动化的场景下启用
- 使用独立的浏览器实例/用户档案，与日常工作浏览器隔离
- 不要在处理敏感数据的浏览器中加载此扩展
...
Arc Tunnel 以 [git submodule](https://git-scm.com/book/en/v2/Git-Tools-Submodules) 形式管理，首次 clone 或更新后需要初始化：

```bash
# 初始化并拉取 submodule
git submodule update --init --recursive

# 后续更新 arc-tunnel 到最新版本
cd external/arc-tunnel && git pull origin main && cd ../..
git add external/arc-tunnel
git commit -m "chore: 更新 arc-tunnel submodule"
```
...
| 端口冲突 / "EADDRINUSE" | 修改 `.env` 中 `ARC_TUNNEL_WS_PORT` 为其他端口（如 `9876`），并在扩展 popup 中同步修改 |
