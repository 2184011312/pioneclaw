...
class TestStoreMemory:
    ...
    async def test_store_success(self, client: AsyncClient, test_user: User):
        resp = await client.post("/api/layered-memory/store", json={
...
class TestRecallMemory:
    ...
    async def test_recall_after_store(self, client: AsyncClient, test_user: User):
        # 先存储
        await client.post("/api/layered-memory/store", json={
        ...
        # 检索
        resp = await client.post("/api/layered-memory/recall", json={
            "query": "编程语言",
            "top_k": 5,
        }, headers=auth_headers(test_user.id))
...
class TestGetMemory:
    ...
    async def test_get_existing(self, client: AsyncClient, test_user: User):
        store_resp = await client.post("/api/layered-memory/store", json={
            "content": "获取测试内容",
            "name": "获取测试",
        }, headers=auth_headers(test_user.id))
        uri = store_resp.json()["uri"]

        resp = await client.get(f"/api/layered-memory/{uri}", headers=auth_headers(test_user.id))
        assert resp.status_code == 200
        data = resp.json()
        assert data["uri"] == uri
    ...
    async def test_get_with_layer(self, client: AsyncClient, test_user: User):
        ...
        # 获取 L0
        resp = await client.get(f"/api/layered-memory/{uri}/.level_0?layer=0", headers=auth_headers(test_user.id))
        assert resp.status_code == 200
        assert resp.json()["layer"] == 0

    @pytest.mark.asyncio
    async def test_get_nonexistent(self, client: AsyncClient, test_user: User):
        resp = await client.get("/api/layered-memory/viking://nonexistent", headers=auth_headers(test_user.id))
...
class TestUpdateMemory:
    ...
    async def test_update_name(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.put(f"/api/layered-memory/{uri}", json={
            "name": "新名",
        }, headers=auth_headers(test_user.id))
        assert resp.status_code == 200
    ...
    @pytest.mark.asyncio
    async def test_update_content_with_tier_regeneration(self, client: AsyncClient, test_user: User):
        store_resp = await client.post("/api/layered-memory/store", json={
            "content": "原始内容",
            "name": "内容更新测试",
        }, headers=auth_headers(test_user.id))
    ...
    async def test_update_nonexistent(self, client: AsyncClient, test_user: User):
        resp = await client.put("/api/layered-memory/viking://nonexistent", json={
            "name": "不存在",
        }, headers=auth_headers(test_user.id))
        assert resp.status_code == 404
...
class TestDeleteMemory:
    ...
    async def test_delete_existing(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.delete(f"/api/layered-memory/{uri}", headers=auth_headers(test_user.id))
        assert resp.status_code == 204

        # 确认已删除
        get_resp = await client.get(f"/api/layered-memory/{uri}", headers=auth_headers(test_user.id))
        assert get_resp.status_code == 404

    @pytest.mark.asyncio
    async def test_delete_nonexistent(self, client: AsyncClient, test_user: User):
        resp = await client.delete("/api/layered-memory/viking://nonexistent", headers=auth_headers(test_user.id))
        assert resp.status_code == 404
...
class TestListMemories:
    ...
    async def test_list_after_store(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.get("/api/layered-memory", headers=auth_headers(test_user.id))
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_list_with_layer_filter(self, client: AsyncClient, test_user: User):
        resp = await client.get("/api/layered-memory?layer=2", headers=auth_headers(test_user.id))
        assert resp.status_code == 200
...
class TestStats:
    ...
    async def test_stats(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.get("/api/layered-memory/stats/overview", headers=auth_headers(test_user.id))
        assert resp.status_code == 200
        data = resp.json()
        assert "total" in data
...
class TestPromote:
    ...
    async def test_promote_l1_to_l2(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.post("/api/layered-memory/promote", json={
            "uri": l1_uri,
        }, headers=auth_headers(test_user.id))
...
class TestEvict:
    ...
    async def test_evict_session(self, client: AsyncClient, test_user: User):
        ...
        resp = await client.post("/api/layered-memory/evict", json={
            "session_id": "test-session-123",
        }, headers=auth_headers(test_user.id))
        assert resp.status_code == 200
