...
class TestAgentAPI:
    ...
    @pytest.mark.asyncio
    async def test_list_agents_success(self, agent_client: AsyncClient, test_user: User):
        """测试获取智能体列表"""
    ...
    @pytest.mark.asyncio
    async def test_create_agent_success(self, agent_client: AsyncClient, test_admin: User):
        """测试创建智能体成功"""
    ...
    @pytest.mark.asyncio
    async def test_get_agent_not_found(self, agent_client: AsyncClient, test_user: User):
        """测试智能体不存在"""
        response = await agent_client.get(
            "/api/agents/99999",
            headers=auth_headers(test_user.id),
