...
def register_channel(channel_type: ChannelType, channel_class: type[BaseChannel]) -> None:
    """注册渠道适配器"""
    _channel_registry[channel_type] = channel_class
    logger.info(f"Registered channel adapter: {channel_type.value}")


def get_channel_class(channel_type: ChannelType) -> Optional[type[BaseChannel]]:
    """获取渠道适配器类"""
...
class ChannelManager:
    ...
    def __init__(self):
        ...
        self._message_handler: Optional[Callable[[ChannelMessage], Any]] = None
        self._started = False
    ...
    async def get(self, channel_id: str) -> Optional[BaseChannel]:
        """获取渠道实例"""
