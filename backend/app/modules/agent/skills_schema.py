...
class SchemaField:
    ...
    # 选择类型
    options: List[Dict[str, str]] = field(default_factory=list)  # [{"value": "x", "label": "Y"}]
...
class SkillsSchemaRegistry:
    ...
    def get_schema(self, skill_name: str) -> Optional[SkillSchema]:
        """获取技能 Schema"""
        ...
        for field_def in fields:
            if field_def.type == "object":
                ...
                if obj or field_def.default is not None:
                    config[field_def.key] = field_def.default if field_def.default is not None else obj
...
_global_schema_registry: Optional[SkillsSchemaRegistry] = None


def get_schema_registry(skills_dir: Optional[Path] = None) -> SkillsSchemaRegistry:
    """获取全局 Schema 注册表实例"""
