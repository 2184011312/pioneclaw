...
class SkillsConfigManager:
    ...
    def __init__(
        self,
        skills_dir: Path,
        schema_registry: Optional[SkillsSchemaRegistry] = None,
    ):
        """
        初始化配置管理器
        ...
        for field_def in fields:
            ...
            if key not in config:
                ...
                if field_def.type == "object":
                    ...
                    obj = {}
                    if field_def.fields:
                        self._add_missing_fields(obj, field_def.fields, changes, f"{full_key}.")
                    config[key] = obj
                ...
                # 递归处理嵌套对象
                self._add_missing_fields(config[key], field_def.fields, changes, f"{full_key}.")
    ...
    def get_field_value(self, skill_name: str, field_key: str) -> Optional[Any]:
        """
        获取配置中的单个字段值
...
_global_config_manager: Optional[SkillsConfigManager] = None


def get_config_manager(skills_dir: Optional[Path] = None) -> SkillsConfigManager:
    """获取全局配置管理器实例"""
