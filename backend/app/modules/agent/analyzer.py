...
class MessageAnalyzer:
    ...
    SKIP_PREFIXES = (
        "好的", "知道了", "明白", "收到", "谢谢", "嗯", "哦",
        "好的", "好", "ok", "OK", "Ok", "嗯嗯", "哦哦", "好好",
        "了解", "可以", "没问题", "是", "是的", "没错", "确实",
        "哈哈", "呵呵", "嘻嘻", "666", "👍", "🙏", "感谢",
        "thanks", "thx", "yes", "no", "yep", "nope", "sure",
        "got it", "noted", "fine", "cool", "nice", "好的呀",
    )
    ...
    def format_messages_for_summary(
        ...
        for msg in messages:
            ...
            # 短消息且以寒暄词开头，跳过
            if len(content) <= 8 and any(content.startswith(p) for p in self.SKIP_PREFIXES):
                continue
    ...
    def should_summarize(
        ...
        total_chars = sum(
            len(self._extract_text(msg.get("content", "")))
            for msg in messages
        )
    ...
    def extract_key_points(self, messages: List[dict]) -> List[str]:
        ...
        for msg in messages:
            ...
            # 提取助手的关键结论
                ...
                # 查找总结性语句
                conclusions = re.findall(r'(因此|所以|总结|结论|建议)[：:]*[^。！？.!?]*', content)
