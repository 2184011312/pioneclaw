...
class WikiSpace(Base):
    ...
    schema: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # 空间自定义字段定义
    settings: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)  # 空间级设置
...
class Wiki(Base):
    ...
    # 分类
    parent_id: Mapped[Optional[str]] = mapped_column(ForeignKey("wikis.id"), nullable=True, index=True)
    ...
    # 空间
    space_id: Mapped[Optional[str]] = mapped_column(ForeignKey("wiki_spaces.id"), nullable=True, index=True)

    # 权限范围
    scope: Mapped[str] = mapped_column(String(20), default="user", index=True)  # system/org/user
    ...
    # 知识库增强字段
    doc_type: Mapped[str] = mapped_column(String(50), default="markdown")  # markdown, text, pdf, url
    ...
    # 关系
    parent: Mapped[Optional["Wiki"]] = relationship(
        "Wiki",
...
class WikiVersion(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    wiki_id: Mapped[str] = mapped_column(ForeignKey("wikis.id"), index=True, nullable=False)
