...
Adapted from skill-creator's aggregate_benchmark.py.
Provides library functions for calculating stats, aggregating results,
and generating benchmark JSON/markdown from either file-system or DB data.
"""
...
def generate_benchmark(benchmark_dir: Path, skill_name: str = "", skill_path: str = "") -> dict:
    """
    Generate complete benchmark.json from run results on the file system.
    """
...
def generate_benchmark_from_db(results: list[dict], skill_name: str) -> dict:
    """
    Generate complete benchmark from database results (instead of file system).

    Args:
        results: List of run result dicts from the database. Each dict should have:
            - configuration: str (e.g. "with_skill", "without_skill")
            - eval_id: int
            - run_number: int
            - pass_rate: float
            - passed: int
            ...
            - errors: int (optional)
            - expectations: list (optional)
            - notes: list (optional)
        skill_name: Name of the skill being benchmarked.

    Returns:
        Benchmark dict with metadata, runs, run_summary, and notes.
    """
    ...
    for result in results:
        ...
        if config not in grouped:
            grouped[config] = []
        grouped[config].append(result)

    run_summary = aggregate_results(grouped)

    # Build runs array for benchmark.json
    runs = []
    for config in grouped:
        for result in grouped[config]:
            runs.append(
                {
                    "eval_id": result["eval_id"],
                    "configuration": config,
    ...
    # Determine runs per configuration
    runs_per_config = max(
        (len(v) for v in grouped.values()), default=0
    )
