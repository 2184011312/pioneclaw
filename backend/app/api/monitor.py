...
from app.models import User, Agent, AgentExecution, Runner, RunnerStatus, Task, ApiUsage
...
async def monitor_stats(
    ...
    # Agents
    agent_online = (await db.execute(select(func.count(Agent.id)).where(Agent.status == "active"))).scalar() or 0
    agent_total = (await db.execute(select(func.count(Agent.id)))).scalar() or 0

    # Runners
    runner_online = (await db.execute(
    ...
    executions_today = (await db.execute(
        select(func.count(AgentExecution.id)).where(AgentExecution.created_at >= today)
    )).scalar() or 0
    ...
    running = (await db.execute(
        select(func.count(Task.id)).where(Task.status.in_(["in_progress", "pending_approval"]))
    )).scalar() or 0
...
async def monitor_executions(
    ...
    return [{
        "id": e.id, "agent_id": e.agent_id, "user_id": e.user_id,
        "status": e.status, "model_name": e.model_name,
        "total_tokens": e.total_tokens, "latency_ms": e.latency_ms,
        "iterations": e.total_iterations, "tool_calls": e.total_tool_calls,
...
async def monitor_execution_detail(
    ...
    return {
        ...
        "output_tokens": e.output_tokens, "latency_ms": e.latency_ms,
        "iterations": e.total_iterations,
        "tool_calls": e.tool_calls, "error_message": e.error_message,
...
async def monitor_runner_status(
    ...
    return {
        ...
        "success_rate": round(
            sum(r.success_tasks or 0 for r in runners) / max(sum(r.total_tasks or 0 for r in runners), 1) * 100, 1
        ),
    }
...
async def monitor_running_tasks(
    ...
    tasks = (await db.execute(
        select(Task).where(Task.status.in_(["in_progress", "pending_approval"])).limit(50)
    )).scalars().all()
    return [{
        "id": t.id, "title": t.title, "status": t.status, "priority": t.priority,
        "assignee_id": t.assignee_id, "started_at": t.started_at.isoformat() if t.started_at else None,
    } for t in tasks]
