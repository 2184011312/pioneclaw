...
async def get_log_stats(
    ...
    success_result = await db.execute(
        ...
        .where(ApiUsage.is_success == True)
    )
    ...
    tokens_result = await db.execute(
        select(func.coalesce(func.sum(ApiUsage.total_tokens), 0))
        .where(ApiUsage.user_id == current_user.id)
    )
    ...
    duration_result = await db.execute(
        select(func.coalesce(func.avg(ApiUsage.duration_ms), 0))
        .where(ApiUsage.user_id == current_user.id)
    )
...
async def delete_log(
    ...
    """删除单条日志"""
    result = await db.execute(
        select(ApiUsage).where(ApiUsage.id == log_id, ApiUsage.user_id == current_user.id)
    )
