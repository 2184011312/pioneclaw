...
<html lang="zh-CN">
<head><meta charset="utf-8"><title>Skill 评估报告 - {skill_name}</title>
<style>
...
<h2>结构检查</h2><table><tr><th>检查项</th><th>状态</th><th>得分</th><th>详情</th></tr>"""]
    ...
    for c in checks:
        passed = c["passed"]
        parts.append(f'<tr><td>{c["check"]}</td><td><span class="badge {"badge-pass" if passed else "badge-fail"}">{"✓ 通过" if passed else "✗ 失败"}</span></td><td>{c["score"]}/{c["max_score"]}</td><td style="font-size:.8em;color:var(--muted)">{c["detail"]}</td></tr>')
    ...
    for r in redflag_hits:
        ...
        parts.append(f'<tr><td style="font-family:monospace;font-size:.8em">{r["rule_id"]}</td><td style="color:{sev_color};font-weight:600;font-size:.75em">{sev}</td><td style="font-size:.85em">{r["description"]}</td><td><span class="badge {"badge-pass" if r["passed"] else "badge-fail"}">{"✓" if r["passed"] else "✕"}</span></td></tr>')
...
现在开始，根据SKILL.md输出JSON数组。"""
    ...
    for attempt in range(2):
        ...
        parts = []
        with open("D:/bigpie/pioneclaw/backend/prompt_gen_debug.txt", "w", encoding="utf-8") as f:
            f.write(f"CALLING LLM (attempt {attempt+1}/2)\n")
        try:
        ...
        with open("D:/bigpie/pioneclaw/backend/prompt_gen_debug.txt", "a", encoding="utf-8") as f:
            f.write("LEN={}\nFIRST={}\nLAST={}\n".format(len(text), text[:300], text[-200:]))
